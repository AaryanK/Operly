...
export function MembersPage({ workspace }: { workspace: WorkspaceSummary }) {
  ...
  async function reload() {
    ...
    try {
      ...
      try {
        setInvitations(await api<WorkspaceInvitation[]>("/workspace/invitations"));
      } catch {
        // Members without workspace:members:manage may still view the members page.
        setInvitations([]);
      }
    } catch (caught) { setError(caught instanceof Error ? caught.message : "Workspace administration is unavailable"); }
    finally { setLoading(false); }
  }
  useEffect(() => { setInviteUrl(null); setNotice(null); reload(); /* eslint-disable-next-line react-hooks/exhaustive-deps */ }, [workspace.id]);
  ...
  return <main className="workspace-page"><header className="surface-header page-header"><div><span className="eyebrow">Administration</span><h1>Members & roles</h1><p>Invite anyone into this workspace, even if they do not have an Operly account yet. Roles become effective only after the person joins.</p></div><div className="page-actions"><button className="secondary-button" onClick={() => setAddRole((value) => !value)}>+ Role</button><button className="primary-button" onClick={() => setAddMember((value) => !value)}>+ Member</button></div></header>
    ...
    {!loading && pendingInvitations.length > 0 && <section className="data-card"><div className="card-heading"><div><span className="eyebrow">Onboarding</span><h2>Pending invitations</h2></div><span>{pendingInvitations.length}</span></div><div className="row-list">{pendingInvitations.map((invitation) => <div className="data-row" key={invitation.id}><div><strong>{invitation.target_email || "One-time share link"}</strong><small>{roles.find((role) => role.key === invitation.role)?.name || invitation.role} · expires {new Date(invitation.expires_at).toLocaleString()}</small></div><button className="secondary-button" onClick={() => revokeInvitation(invitation.id)}>Revoke</button></div>)}</div><form className="inline-form" onSubmit={createOpenInvite}><label>Share-link role<select name="role">{roles.map((role) => <option key={role.key} value={role.key}>{role.name}</option>)}</select></label><button className="secondary-button">Create one-time share link</button><small>Use this when you want to invite someone from an external workspace without requiring their email first.</small></form></section>}
    {!loading && pendingInvitations.length === 0 && invitations.length >= 0 && <form className="inline-form" onSubmit={createOpenInvite}><label>Share-link role<select name="role">{roles.map((role) => <option key={role.key} value={role.key}>{role.name}</option>)}</select></label><button className="secondary-button">Create one-time share link</button><small>Useful for Discord, Slack, or another external workspace when you want the participant to claim membership themselves.</small></form>}
