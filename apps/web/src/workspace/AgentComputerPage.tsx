...
export function AgentComputerPage({ workspace }: { workspace: WorkspaceSummary }) {
  ...
  const [networkPolicy, setNetworkPolicy] = useState<"off" | "web">("web");
    ...
    <main className="workspace-page agent-computer-page">
      <header className="surface-header page-header agent-computer-header">
        <div>
          ...
          <p>
            Each run gets an isolated Railway Sandbox VM with Python, terminal, files, processes, Git, public web and browser tools.
            Operly business actions stay separate governed Workspace capabilities, so compute access never becomes hidden business authority.
          </p>
        </div>
        <div className="page-actions">
          <span className={`status-chip ${status?.runner_configured ? "status-active" : ""}`}>{status?.runner_configured ? "Sandbox Runner configured" : "Sandbox Runner required"}</span>
      ...
      {loading ? <div className="loading-panel">Starting Agent Computer…</div> : (
        <div className="agent-computer-layout">
              ...
              {!active ? (
                <form className="computer-task-composer" onSubmit={createAndRun}>
                  <div className="computer-screen-intro">
                    ...
                    <div><span className="eyebrow">Railway Sandbox workspace</span><h2>Start a Computer</h2><p>Use a general isolated VM for coding/research/data work, or choose a Studio preset. The agent uses the same native and Workspace tools exposed here.</p></div>
                  </div>
                  ...
                  <div className="computer-runtime-options">
                    ...
                    <label>Network
                      <select value={networkPolicy} onChange={(event) => setNetworkPolicy(event.target.value as typeof networkPolicy)}>
                        <option value="off">Off</option><option value="web">Public web only</option>
                      </select>
                    </label>
                  </div>
                  ...
                  {action === "general" && !status?.runner_configured && <small className="computer-guardrail-note">Configure Operly Sandbox Runner before starting agent compute. Operly never falls back to the API server shell.</small>}
                ...
                <div className="computer-session-view">
                  ...
                  <article className="computer-result-card">
                    <div className="card-heading"><div><span className="eyebrow">Railway Sandbox runtime</span><h3>{runtimeActive ? "Computer is active" : `Computer is ${active.runtime.state || "stopped"}`}</h3></div><span className={`status-chip ${runtimeActive ? "status-active" : ""}`}>{active.runtime.profile}</span></div>
                    <p>{runtimeActive ? "Python, terminal, files, processes, Git, public web and browser tools are running in the session's isolated Railway Sandbox VM." : "Start an isolated Railway Sandbox runtime to use native compute tools. Studio/business preset results remain available independently."}</p>
          ...
          <aside className="data-card computer-authority-panel">
            ...
            <div className="computer-boundary-note"><strong>Runtime boundary</strong><p>Agent → Computer-native tools → Railway Sandbox VM. Business side effects take a separate path: Agent → Workspace capability → permissions/approval → provider.</p></div>
            <div className="computer-boundary-note"><strong>Isolation</strong><p>The sandbox gets no Operly connector secrets and is not joined to Operly's private service network. The API server never becomes the agent's shell.</p></div>
          </aside>
        </div>
      )}
    </main>
