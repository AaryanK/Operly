...
export function WorkspaceHome({ workspace }: Props) {
  ...
  useEffect(() => {
    Promise.allSettled([api<Row[]>("/tasks"), api<Row[]>("/approvals"), api<Row[]>("/solutions")]).then(([taskResult, approvalResult, solutionResult]) => {
      ...
      if ([taskResult, approvalResult, solutionResult].every((item) => item.status === "rejected")) setError("Workspace overview is temporarily unavailable.");
    });
  }, [workspace.id]);
  ...
  return (
    <main className="workspace-page">
      <header className="surface-header workspace-hero page-header">
        <div><span className="eyebrow">Workspace</span><h1>{workspace.name}</h1><p>Home is the operating overview. AI conversations belong in Operly; software creation belongs in Solutions.</p></div>
