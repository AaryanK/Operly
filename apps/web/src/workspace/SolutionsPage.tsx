...
export function SolutionsPage({ workspace }: { workspace: WorkspaceSummary }) {
  ...
  return <main className="workspace-page">
    ...
    <section className="solutions-library"><div className="section-heading"><div><span className="eyebrow">Workspace</span><h2>Your Solutions</h2></div><span>{solutions.length}</span></div>{loading ? <div className="loading-panel">Loading Solutions…</div> : solutions.length ? <div className="solution-grid">{solutions.map((solution) => { const preview = object(solution.preview); const production = object(solution.production); return <article className="solution-card" key={text(solution.id)}><div className="solution-card-top"><span>{title(solution.solution_type || solution.type || "Solution")}</span><span className={`status-chip status-${text(solution.status, "unknown").replaceAll("_", "-")}`}>{title(solution.status || "unknown")}</span></div><h3>{text(solution.name, "Untitled Solution")}</h3><p>{text(solution.objective || solution.description, "Workspace Solution")}</p><div className="solution-meta"><span>Preview: {text(preview.state || (preview.url ? "available" : "not ready"), "not ready")}</span><span>Production: {text(production.state, "not published")}</span></div><div className="page-actions">{text(preview.url) && <a className="secondary-button" href={text(preview.url)} target="_blank" rel="noreferrer">Preview</a>}{text(production.url) && <a className="primary-button" href={text(production.url)} target="_blank" rel="noreferrer">Live</a>}</div></article>; })}</div> : <div className="empty-panel">No Solutions yet. Create one from the business objective above.</div>}</section>
