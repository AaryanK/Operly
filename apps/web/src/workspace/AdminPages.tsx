...
export function MembersPage({ workspace }: { workspace: WorkspaceSummary }) {
  ...
  async function createRole(event: FormEvent<HTMLFormElement>) {
    event.preventDefault(); const form = new FormData(event.currentTarget);
    const permissions = roles.flatMap((role) => rows(role.permissions).map(String));
    try { await api("/workspace/roles", { method: "POST", body: JSON.stringify({ name: form.get("name"), key: form.get("key") || null, permissions: [...new Set(permissions)].slice(0, 0) }) }); setAddRole(false); await reload(); }
