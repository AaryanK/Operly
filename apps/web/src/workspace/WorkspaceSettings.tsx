...
export function WorkspaceSettings({ workspace, onRefresh }: Props) {
  ...
  async function loadConnectors() {
    try {
      setConnectors(await api<WorkspaceConnector[]>("/connectors"));
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : "Workspace connections are unavailable");
    }
  }

  useEffect(() => { loadConnectors(); }, [workspace.id]);
  ...
  async function connectCanva() {
    setBusy(true); setError(null); setMessage(null);
    try {
      const result = await api<{ authorization_url: string }>("/connectors/canva/connect", { method: "POST", body: "{}" });
  ...
  return <main className="workspace-page">
    <header className="surface-header page-header"><div><span className="eyebrow">Administration</span><h1>Workspace settings</h1><p>Workspace identity and workspace-owned connections stay inside this shared boundary. Personal account connections remain private to you.</p></div></header>
    <section className="settings-grid">
      ...
      <article className="data-card settings-card full-settings"><div className="card-heading"><div><span className="eyebrow">Connections</span><h2>Workspace tools</h2><p>These credentials belong to this workspace and are available only through its authorized capability boundary.</p></div>{workspace.role === "owner" && <button className="primary-button" disabled={busy} onClick={connectCanva}>Connect Canva</button>}</div>{workspace.role !== "owner" && <div className="empty-panel">Only the workspace owner can connect or remove workspace tools.</div>}{connectors.length ? <div className="settings-section-stack">{connectors.map((connector) => <article className="personal-connector-card" key={connector.id} data-provider={connector.provider}><span className="connector-logo" aria-hidden="true">{connector.provider.slice(0,1).toUpperCase()}</span><div><strong>{connector.display_name}</strong><p>{connector.account || connector.provider}</p><small>{(connector.capabilities || []).slice(0,5).join(" · ") || "No exposed capabilities"}</small>{connector.last_error && <span className="form-error">{connector.last_error}</span>}</div><span className={`status-chip status-${connector.health_status || connector.status}`}>{connector.health_status || connector.status}</span>{workspace.role === "owner" && <div className="row-actions"><button className="secondary-button" disabled={busy} onClick={() => connectorAction(() => api(`/connectors/${connector.id}/test`, { method: "POST", body: "{}" }), `${connector.display_name} connection tested.`)}>Test</button><button className="danger-button" disabled={busy} onClick={() => connectorAction(() => api(`/connectors/${connector.id}`, { method: "DELETE" }), `${connector.display_name} disconnected.`)}>Disconnect</button></div>}</article>)}</div> : <div className="empty-panel">No workspace OAuth connectors yet.</div>}</article>
