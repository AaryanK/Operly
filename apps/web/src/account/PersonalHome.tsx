...
export function PersonalHome({ profile }: Props) {
  ...
  useEffect(() => {
    ...
    // The first load intentionally owns conversation selection.
    ...
    <div className="personal-layout">
      <aside className="personal-history">
        <div className="history-head">
          <div><small>YOUR SPACE</small><strong>Personal Operly</strong></div>
          <button onClick={() => { setConversationId(null); setMessages([]); setError(null); }} aria-label="New conversation">+</button>
        </div>
        <div className="history-list">
          {conversations.length === 0 && <p className="empty-copy">Your private conversations will appear here.</p>}
          {conversations.map((item) => (
            <button key={item.id} className={conversationId === item.id ? "active" : ""} onClick={() => openConversation(item.id)}>
              <span>✦</span>
              <span><strong>{item.title || "Conversation"}</strong><small>{formatDate(item.updated_at)}</small></span>
            </button>
          ))}
        </div>
      ...
      <main className="personal-surface">
        <header className="surface-header">
          <div>
            ...
            <h1>Operly</h1>
            <p>Your account-level AI. This transcript stays personal; workspace tools are reached only through permission-checked account capabilities.</p>
          </div>
        ...
        <div className="conversation-stage" ref={stage} aria-live="polite">
          {messages.length === 0 && (
            <article className="assistant-message">
              ...
              <div><strong>Operly</strong><p>I’m your private Operly. Ask across your account, attach a file, or tell me which workspace you want me to work with.</p></div>
            </article>
          )}
          {messages.map((item, index) => (
            <article className={`chat-message ${item.role}`} key={item.id || `${item.role}-${index}`}>
              <span className="assistant-avatar">{item.role === "assistant" ? "✦" : "Y"}</span>
              <div><strong>{item.role === "assistant" ? "Operly" : "You"}</strong><p>{item.content}</p></div>
            </article>
          ))}
        ...
        <form className="composer" onSubmit={submit}>
          ...
          <div className="composer-actions">
            <div><button type="button" className="attach-button" onClick={() => fileInput.current?.click()}>＋ Attach</button><span>Private account scope</span></div>
            <button disabled={busy || (!message.trim() && !files.length)}>{busy ? "Working…" : "Send"}</button>
          </div>
        </form>
      </main>
    </div>
