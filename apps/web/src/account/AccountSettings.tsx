...
export function AccountSettings({ profile, workspaces, initialTab = "account", themePreference, resolvedTheme, onThemePreference, onClose, onRefresh, onWorkspace }: Props) {
  ...
  async function connectCanva() {
    setBusy(true); setError(null);
    try { const result = await api<{ authorization_url: string }>("/personal-connectors/canva/connect", { method: "POST", body: "{}" }); window.location.assign(result.authorization_url); }
    catch (caught) { setError(caught instanceof Error ? caught.message : "Canva authorization could not start"); setBusy(false); }
  }
