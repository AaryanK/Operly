...
export function AccountSettings({ profile, workspaces, initialTab = "account", themePreference, resolvedTheme, onThemePreference, onClose, onRefresh, onWorkspace }: Props) {
  ...
  useEffect(() => { if (tab === "connections") void loadConnections(); }, [tab]);
  ...
  async function createWorkspace(event: FormEvent<HTMLFormElement>) {
    ...
    setBusy(true); setError(null); setMessage(null);
    try {
      const created = await api<{ ok: boolean; workspace: WorkspaceSummary }>("/auth/workspaces", {
        method: "POST",
  ...
  function connectDiscord() { window.location.assign("/api/identities/discord/sign-in"); }
  ...
  const identityLabel = profile?.display_name || profile?.email || "Operly user";

  return <div className="editor-overlay account-settings-overlay">
    <button className="editor-backdrop" onClick={onClose} aria-label="Close account settings"></button>
    <section className="account-settings-card" role="dialog" aria-modal="true" aria-label="Account settings">
      <aside className="account-settings-nav">
        <div><span className="eyebrow">Personal</span><strong>User settings</strong></div>
