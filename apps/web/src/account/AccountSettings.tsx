...
export function AccountSettings({ profile, workspaces, initialTab = "account", onClose, onRefresh, onWorkspace }: Props) {
  ...
  async function createWorkspace(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const created = await api<WorkspaceSummary>("/workspaces", { method: "POST", body: JSON.stringify({ name: form.get("name"), timezone: form.get("timezone") || "UTC" }) });
      await onRefresh(); onClose(); await onWorkspace(created.id);
