...
function validatePayload(payload) {
  ...
  if (mode === "python") {
    ...
    return { mode, decoded, outputs, timeoutSeconds, code };
  }
  ...
  return { mode, decoded, outputs, timeoutSeconds, argv };
}

async function computerTemplate() {
  return Sandbox.template()
    .withPackages("python3", "nodejs", "ffmpeg", "poppler-utils", "ca-certificates", "iptables")
...
async function executeComputer(payload) {
  ...
  const template = await computerTemplate();
  ...
  try {
    // Defense in depth: generated/user-authored processes run as uid 10001 and
    // cannot egress even if the provider-level isolation contract changes.
    await box.exec(
    ...
    if (request.mode === "python") {
      ...
      command = "python3 /workspace/work/agent.py";
    } else {
...
async function handleComputer(req, res) {
  try {
    ...
    const result = await executeComputer(payload);
...
export function installAgentComputerEndpoint() {
  ...
  http.createServer = function patchedCreateServer(options, listener) {
    ...
    const wrapped = (req, res) => {
      ...
      if (pathname === "/v1/computer/execute") {
        void handleComputer(req, res);
        return;
      }
...
export { validatePayload, safeRelative };
