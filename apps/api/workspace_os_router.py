...
from sqlalchemy import Boolean, DateTime, Float, Integer, String, delete, func, or_, select
...
ENTITY_REGISTRY: dict[str, RecordConfig] = {
    ...
    "orders": RecordConfig(
        ...
        search_fields=("status", "notes"), references=(("contact_id", Contact),),
    ),
    "quotes": RecordConfig(
    ...
    "appointments": RecordConfig(
        ...
        required=("title", "starts_at"), search_fields=("title", "status", "assigned_to"), references=(("contact_id", Contact),),
    ...
    "integrations": RecordConfig(
        ...
        required=("connector_type", "provider", "display_name"), search_fields=("provider", "display_name", "status", "health_status"),
    ),
}
...
async def _require_module(db: AsyncSession, auth: AuthContext, key: str, permission: str) -> set[str]:
    ...
    if not await _module_enabled(db, auth.tenant.id, key):
        raise HTTPException(
            status_code=409,
            detail={"code": "MODULE_DISABLED", "message": f"Enable the {module_manifest(key)['name']} module first."},
        )
    return permissions
...
async def set_module_state(
    ...
    row = await _upsert_module(db, auth, key, enabled=payload.enabled, configuration=payload.configuration)
    await db.commit()
    return {"ok": True, "key": key, "enabled": row.enabled, "state": row.state, "configuration": json.loads(row.configuration_json or "{}")}
...
async def add_member(
    ...
    if user is None or not user.active:
        raise HTTPException(status_code=404, detail="That email does not have an active Operly account yet")
...
async def set_role_permissions(
    ...
    if row is None:
        row = WorkspaceRole(
            ...
            key=key,
            name=_clean_text(payload.name or key.replace("-", " ").title(), max_length=120),
...
async def summary(
    ...
    low_stock = int(await db.scalar(select(func.count(CatalogItem.id)).where(CatalogItem.tenant_id == tenant_id, CatalogItem.active.is_(True), CatalogItem.stock_qty <= CatalogItem.reorder_level)) or 0)
    return {
        "contacts": await count(Contact),
        "open_leads": int(await db.scalar(select(func.count(Lead.id)).where(Lead.tenant_id == tenant_id, Lead.stage.notin_(["won", "lost"]))) or 0),
        ...
        "low_stock": low_stock,
        "open_tickets": int(await db.scalar(select(func.count(SupportTicket.id)).where(SupportTicket.tenant_id == tenant_id, SupportTicket.status.notin_(["resolved", "closed"]))) or 0),
        "upcoming_appointments": int(await db.scalar(select(func.count(Appointment.id)).where(Appointment.tenant_id == tenant_id, Appointment.starts_at >= datetime.utcnow(), Appointment.status != "cancelled")) or 0),
    }
...
async def list_records(
    ...
    query = select(config.model).where(config.model.tenant_id == auth.tenant.id)
    ...
    if status and hasattr(config.model, "status"):
        query = query.where(config.model.status == status)
    ...
    count_query = select(func.count(config.model.id)).where(config.model.tenant_id == auth.tenant.id)
...
async def inventory_movements(
    ...
    return [
        {"id": row.id, "quantity_change": row.quantity_change, "reason": row.reason, "created_at": row.created_at.isoformat()}
        for row in rows
    ]
