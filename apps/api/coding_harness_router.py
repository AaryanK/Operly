...
from packages.coding_harness.build_service import SourceRecordError, submit_source_build
...
from packages.coding_harness.source_service import generate_source_for_plan, latest_source, source_record_json
...
async def create_harness_source(
    plan_id: str,
    payload: GenerateApprovedPlanInput,
    ...
    try:
        ...
        await db.rollback()
        raise HTTPException(status_code=422, detail={"code": "coding_harness_blocked", "message": str(error)}) from error


@router.get("/api/coding-harness/plans/{plan_id}/source")
async def get_harness_source(
    plan_id: str,
    auth: AuthContext = Depends(get_auth_context),
    db: AsyncSession = Depends(get_db),
):
...
async def create_harness_build(
    payload: RunnerBuildInput,
    auth: AuthContext = Depends(get_auth_context),
    db: AsyncSession = Depends(get_db),
):
    """Build the latest harness-authored source only inside the isolated runner."""
    _assert_owner(auth)
    row, plan = await _approved_plan(db, auth, payload.planId, payload.approvedVersion)
    source = await latest_source(db, auth.tenant.id, row.id, payload.approvedVersion)
    if source is None:
        try:
            source, _ = await generate_source_for_plan(db, auth.tenant.id, auth.user.id, row, plan)
            await db.commit()
    ...
    if build.state == "preview_ready":
        preview = await db.scalar(
            select(RunnerPreviewRecord).where(
                RunnerPreviewRecord.build_id == build.id,
                RunnerPreviewRecord.tenant_id == auth.tenant.id,
        ...
        if preview:
            result["preview"] = {
                "id": preview.id,
                "url": f"/api/custom-software/previews/{preview.id}/",
