...
from packages.workspace_modules.integrations.canva.authoring import SCOPE_BY_CAPABILITY, workspace_canva_authoring_capabilities
...
class WorkspaceIntegrationPackageTests(unittest.TestCase):
    ...
    def test_canva_requires_exact_external_scopes(self):
        ...
        self.assertEqual(SCOPE_BY_CAPABILITY["canva.design.dataset"], frozenset({"design:content:read"}))
        self.assertEqual(SCOPE_BY_CAPABILITY["canva.brand_templates.list"], frozenset({"brandtemplate:meta:read"}))
        self.assertEqual(SCOPE_BY_CAPABILITY["canva.brand_template.dataset"], frozenset({"brandtemplate:content:read"}))
        self.assertEqual(SCOPE_BY_CAPABILITY["canva.autofill.create"], frozenset({"design:content:write"}))

    def test_external_content_mutations_are_approval_gated(self):
        canva = {spec.id: spec for spec in workspace_canva_capabilities()}
        canva_authoring = {spec.id: spec for spec in workspace_canva_authoring_capabilities()}
        ...
        self.assertEqual(canva_authoring["canva.autofill.create"].risk, CapabilityRisk.HIGH)
    ...
    def test_canva_authoring_has_real_dataset_and_autofill_boundary(self):
    ...
    def test_provider_write_permissions_are_operly_permissions(self):
        ...
        self.assertEqual(canva_authoring["canva.autofill.create"].permissions, ("marketing:write",))
