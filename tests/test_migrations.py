...
class MigrationTests(unittest.TestCase):
    ...
    def test_fresh_upgrade_and_idempotency(self):
        path=self.root/"fresh.db";self.upgrade(path);self.upgrade(path);self.assertEqual(revisions(url(path))[0],"0016_operating_product")
    ...
    def test_upgrade_repairs_create_all_polluted_0002_database(self):
        ...
        self.upgrade(path)
        with closing(sqlite3.connect(path)) as db:
            self.assertEqual(db.execute("select version_num from alembic_version").fetchone()[0],"0016_operating_product")
            columns={row[1] for row in db.execute("pragma table_info(generated_projects)")}
            self.assertTrue({"plan_id","approved_plan_version","architecture_pack"}<=columns)
...
class StartupRevisionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_current_database_is_accepted(self):
        ...
        async with engine.begin() as connection:
            ...
            await connection.exec_driver_sql("INSERT INTO alembic_version VALUES('0016_operating_product')")
