...
class MigrationTests(unittest.TestCase):
    ...
    def test_fresh_upgrade_and_idempotency(self):
        path=self.root/"fresh.db";self.upgrade(path);self.upgrade(path);self.assertEqual(revisions(url(path))[0],"0004_managed_record_runtime")
    ...
    def test_unsafe_orphan_fails_without_claiming_head(self):
        ...
        self.assertNotEqual(revisions(url(path))[0],"0004_managed_record_runtime")
...
class StartupRevisionTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_current_database_is_accepted(self):
        ...
        async with engine.begin() as connection:
            ...
            await connection.exec_driver_sql("INSERT INTO alembic_version VALUES('0004_managed_record_runtime')")
