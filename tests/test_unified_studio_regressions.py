...
class UnifiedStudioRegressionTests(unittest.IsolatedAsyncioTestCase):
    ...
    def test_logout_requires_confirmed_server_logout_before_returning_to_login(self):
        # The canonical signed-in renderer is React. Keep this regression pinned
        # to AccountSettings, where the visible sign-out action now owns logout.
        source = (ROOT / "apps" / "web" / "src" / "account" / "AccountSettings.tsx").read_text("utf-8")
        ...
        self.assertLess(source.index(request), source.index(navigate, source.index(request)))
        self.assertIn("Sign out failed", source)
        self.assertNotIn('finally { window.location.assign("/login")', source)
