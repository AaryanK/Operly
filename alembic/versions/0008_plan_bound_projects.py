...
def upgrade():
    ...
    foreign_keys={tuple(item.get("constrained_columns") or ()) for item in inspector.get_foreign_keys("generated_projects")}
    with op.batch_alter_table("generated_projects") as batch:
        ...
        if ("plan_id",) not in foreign_keys:batch.create_foreign_key("fk_generated_projects_plan","software_plans",["plan_id"],["id"])
