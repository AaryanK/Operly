...
def upgrade():
    ...
    for name, cols in [
        ...
        ("ix_workflow_runs_authority_user_id", ["authority_user_id"]),
    ...
    inspector = sa.inspect(op.get_bind())
    if not inspector.has_table("workflow_step_attempts"):
        op.create_table(
            ...
            sa.Column("finished_at", sa.DateTime(), nullable=True),
            sa.UniqueConstraint("step_run_id", "attempt", name="uq_workflow_step_attempt_number"),
        )
    for name, cols in [
        ("ix_workflow_step_attempts_workflow_run_id", ["workflow_run_id"]),
        ("ix_workflow_step_attempts_step_run_id", ["step_run_id"]),
        ("ix_workflow_step_attempts_capability_id", ["capability_id"]),
        ("ix_workflow_step_attempts_status", ["status"]),
        ("ix_workflow_step_attempts_request_id", ["request_id"]),
        ("ix_workflow_step_attempts_kernel_run_id", ["kernel_run_id"]),
        ("ix_workflow_step_attempts_approval_id", ["approval_id"]),
        ("ix_workflow_step_attempts_created_at", ["created_at"]),
    ]:
        _index(name, "workflow_step_attempts", cols)
    ...
    if not inspector.has_table("workflow_trace_events"):
        op.create_table(
            ...
            sa.Column(
                "step_attempt_id",
                sa.String(length=36),
                sa.ForeignKey("workflow_step_attempts.id", ondelete="CASCADE"),
                nullable=True,
            ),
    ...
    for name, cols in [
        ...
        ("ix_workflow_trace_events_step_attempt_id", ["step_attempt_id"]),
