...
from packages.custom_software.live_planning import (LivePlanningOrchestrator,OllamaPlanningClient,PlanningMode,
...
async def create_plan(db,tenant_id,user_id,prompt):
    ...
    row=SoftwarePlanRecord(tenant_id=tenant_id,prompt=prompt,created_by=user_id,status="planning");db.add(row);await db.flush()
    if mode==PlanningMode.LIVE_LLM:
        ...
        orchestrator=LivePlanningOrchestrator(OllamaPlanningClient(),on_result=persist_result)
        ...
        data=planned.model_dump();data["planningMode"]="deterministic_test";data["planningMetrics"]["planningMode"]="deterministic_test"
    ...
    row.status="draft"
    version=SoftwarePlanVersion(tenant_id=tenant_id,plan_id=row.id,version=1,plan_json=planned.model_dump_json(),requirement_ledger_json=json.dumps([x.model_dump() for x in planned.requirementLedger]),plan_tree_json=json.dumps([x.model_dump() for x in planned.planTree]),validation_json=json.dumps(planned.globalValidation),semantic_diff_json=planned.semanticDiff.model_dump_json() if planned.semanticDiff else "{}",created_by=user_id);db.add(version);await db.commit();await db.refresh(row);return row,version,planned

def _invocation(row,tenant_id,role,node_id,result):
    return PlanningModelInvocation(tenant_id=tenant_id,plan_id=row.id,plan_version=1,node_id=node_id,role=role,planning_mode="live_llm",provider=result.provider,model_id=result.model_id,request_id=result.request_id,context_digest=result.context_digest,attempt=result.attempt,structured_output_json=json.dumps(result.structured_output or {}),raw_response=result.raw_response,validation_errors_json=json.dumps(result.validation_errors),retry_history_json=json.dumps(result.retry_history),latency_ms=result.latency_ms,input_tokens=result.input_tokens,output_tokens=result.output_tokens,failure_classification=result.failure_classification)

def _live_plan(prompt,outcome):
    base=build_software_plan(prompt).model_dump(); analysis=outcome["analysis"]; nodes=outcome["nodes"]; validations=outcome["validations"]; budget=outcome["budget"]
    ledger=[]
    for req in analysis.requirements:
        linked=[x.node_id for x in nodes if req.requirement_id in x.linked_requirement_ids]
        ledger.append({"id":req.requirement_id,"originalSource":"original_request","exactText":req.source_excerpt,"normalizedMeaning":req.normalized_requirement,"mandatory":req.priority.lower()!="optional","category":req.category,"acceptanceCriteria":req.acceptance_criteria,"relatedPlanNodeIds":linked,"relatedArtifactIds":[a for x in nodes if req.requirement_id in x.linked_requirement_ids for a in x.required_artifacts],"relatedTestIds":[t for x in nodes if req.requirement_id in x.linked_requirement_ids for t in x.required_tests],"coverageStatus":"implementation_ready" if linked else "unplanned","verificationStatus":"unverified","planningMode":"live_llm","explicitTerms":req.explicit_terms,"exclusions":req.exclusions,"ambiguities":req.ambiguities,"conflicts":req.conflicts,"assumptions":req.assumptions})
    tree=[]
    for node in nodes:
    ...
    base["provenance"]={**base.get("provenance",{}),"planningMode":"live_llm","providerModel":f"{invocations[0][2].provider}/{invocations[0][2].model_id}" if invocations else "unknown","invocationCount":len(invocations),"contextPacketsStoredAsDigests":True}
...
async def revise(db,row,user_id,request,expected):
    ...
    _,current=await plan_version(db,row);updated=revise_plan(current,request)
    if not updated.semanticDiff or not updated.semanticDiff.structuralChange:raise PlanConflict("Revision produced no structural plan change")
    row.current_version+=1;row.status="draft";row.approved_version=None
    version=SoftwarePlanVersion(tenant_id=row.tenant_id,plan_id=row.id,version=row.current_version,plan_json=updated.model_dump_json(),requirement_ledger_json=json.dumps([x.model_dump() for x in updated.requirementLedger]),plan_tree_json=json.dumps([x.model_dump() for x in updated.planTree]),validation_json=json.dumps(updated.globalValidation),semantic_diff_json=updated.semanticDiff.model_dump_json() if updated.semanticDiff else "{}",revision_request=request,created_by=user_id);db.add(version);await db.commit();await db.refresh(row);return version,updated
