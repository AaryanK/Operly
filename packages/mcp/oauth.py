...
async def consume_refresh_token(
    ...
    *,
    ...
    Reuse of a superseded generation revokes the entire grant, which prevents an
    attacker who won a refresh-token race from silently retaining the replacement.
    """
    ...
    if not supplied_generation or not hmac.compare_digest(supplied_generation, current_generation):
        # A rotated refresh token being presented again is evidence that the token
        # family may have been copied. Persist revocation before returning the error;
        # otherwise request-session rollback would undo the protective action.
        grant.status = "revoked"
        grant.updated_at = now
        await db.commit()
        raise McpOAuthError("MCP refresh token reuse detected; client grant revoked")
    ...
    previous_stamp = grant.updated_at or grant.created_at
    grant.updated_at = now if now > previous_stamp else previous_stamp + timedelta(microseconds=1)
    await db.flush()
