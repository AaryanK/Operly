"""Bounded build-test-diagnose-repair loop for canonical SoftwareProject source.

Generated code never executes in the OPERLY control plane. All software surfaces use
this same coding/runner loop; there is no second Studio-owned orchestration runtime.
"""
...
from packages.software_projects.coding.build_service import (
    RunnerProfileUnsupported,
    submit_source_build,
)
from packages.software_projects.coding.source_service import (
...
def _repair_budget(value: int | None = None) -> int:
    configured = (
        int(os.getenv("OPERLY_CODING_REPAIR_ATTEMPTS", "2"))
        if value is None
        else int(value)
    )
...
def _runner_poll_interval() -> float:
    try:
        return max(
            0.25,
            min(float(os.getenv("OPERLY_RUNNER_POLL_INTERVAL_SECONDS", "1")), 10.0),
        )
    except ValueError:
        return 1.0
...
def _runner_poll_timeout() -> float:
    try:
        return max(
            30.0,
            min(float(os.getenv("OPERLY_RUNNER_POLL_TIMEOUT_SECONDS", "900")), 3600.0),
        )
    except ValueError:
        return 900.0
...
def _trace_metadata(
    ...
    """Correlate the coding subtask with its owning Solution generation attempt."""
    solution_id = ""
    attempt = 1
    prefix = "solution:"
    marker = ":software-build:"
    ...
    conversation_id = (
        f"solution:{solution_id}" if solution_id else f"software-plan:{plan_id}"
    )
    run_id = (
        f"solution:{solution_id}:attempt:{attempt}"
        if solution_id
        else f"software-plan:{plan_id}"
    )
    return {
        ...
        "runtime_component": "software_coding",
...
async def _trace(
    ...
    *,
    ...
    resource_id: str = "software_coding",
) -> None:
...
async def _await_runner_build(
    ...
    if build.state in SETTLED_BUILD_STATES:
        ...
        await _progress(
            progress_callback,
            _stage_for_build(build),
            "settled",
            evidence,
        )
        await _trace(
            "runner.build.settled",
            evidence,
    ...
    await _progress(
        progress_callback,
        _stage_for_build(build),
        "succeeded" if build.state == "preview_ready" else "failed",
        evidence,
    )
    await _trace(
        "runner.build.settled",
...
async def _repair(
    ...
    await _trace(
        "software_coding.repair.started",
        payload,
    ...
    await _trace(
        "software_coding.repair.completed",
        completed_payload,
        resource_id="coding_agent",
    )
    return source
...
async def build_with_repair(
    ...
    *,
    ...
    AgentRuntime owns the root objective; this helper owns only the bounded
    SoftwareProject coding and isolated-runner subtask.
    """
    ...
    with runtime_trace_scope(metadata):
        ...
        try:
            ...
            if source is None:
                ...
                await _trace(
                    "coding_agent.source_generation.started",
                    {"planId": getattr(plan_row, "id", None)},
                    resource_id="coding_agent",
                )
                ...
                await _trace(
                    "coding_agent.source_generation.completed",
                    payload,
                    resource_id="coding_agent",
                )
            else:
            ...
            while True:
                attempt_key = (
                    idempotency_key
                    if used_repairs == 0
                    else f"{idempotency_key}-repair-{used_repairs}"
                )
                try:
                    submit_payload = {
                    ...
                    await _trace(
                        "runner.submit.persisted",
                        {
                            ...
                            "classification": getattr(
                                build, "failure_classification", None
                            ),
                        },
                        resource_id="sandbox_runner",
                    )
                    build = await _await_runner_build(
                        db,
                        build,
                        adapter,
                        progress_callback,
                    ...
                    evidence = {
                        ...
                        "instruction": (
                            "Preserve product behavior but adapt the source tree to one "
                            "supported isolated runtime profile using only its declared "
                            "standard-library/dependency-free contract."
                        ),
                    }
                    source = await _repair(
                        db,
                        tenant_id,
                        user_id,
                        plan_row,
                ...
                evidence = _failure_evidence(build)
                classification = str(
                    evidence.get("classification") or "unknown_failure"
                )
                if used_repairs >= budget or classification not in REPAIRABLE_FAILURES:
                    await _trace(
                        "software_coding.failed",
                        evidence,
                        phase="error",
                        classification=classification,
                        retryable=True,
                ...
                source = await _repair(
                    ...
                    repairs,
                    used_repairs,
                    failed_build_id=getattr(build, "id", None),
                    progress_callback=progress_callback,
                )
        except Exception as error:
            await _trace(
                "software_coding.exception",
