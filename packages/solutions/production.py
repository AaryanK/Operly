...
from packages.studio.source_agent import mark_published, production_html
...
class ProductionService:
    ...
    async def _version(self,db,tenant_id,runtime,version_reference=None):
        source=None;legacy=None
        if version_reference:
            source=await db.scalar(select(StudioSourceVersion).where(StudioSourceVersion.id==version_reference,StudioSourceVersion.project_id==runtime.id,StudioSourceVersion.tenant_id==tenant_id))
            if source is None:
                legacy=await db.scalar(select(StudioVersion).where(StudioVersion.id==version_reference,StudioVersion.project_id==runtime.id,StudioVersion.tenant_id==tenant_id))
        else:
            source=await db.scalar(select(StudioSourceVersion).where(StudioSourceVersion.project_id==runtime.id,StudioSourceVersion.tenant_id==tenant_id).order_by(desc(StudioSourceVersion.source_version)))
            if source is None and runtime.active_draft_version_id:
                legacy=await db.scalar(select(StudioVersion).where(StudioVersion.id==runtime.active_draft_version_id,StudioVersion.project_id==runtime.id,StudioVersion.tenant_id==tenant_id))
        if source is None and legacy is None:raise LookupError("Verified version not found")
    ...
    async def publish(self,db,tenant_id,solution_id,user_id,*,idempotency_key=None,version_reference=None,job_type=JobType.PUBLISH):
        ...
        source,legacy=await self._version(db,tenant_id,runtime,version_reference)
        version_id=source.id if source else legacy.id
        source_kind="source_agent" if source else "legacy_schema"
        key=idempotency_key or f"{solution_id}:{version_id}:{job_type}";job,created=await self._job(db,tenant_id,solution_id,version_id,job_type,key)
        if not created:return job,row
        event_prefix="solution.rollback" if job_type==JobType.ROLLBACK else "solution.publish"
        await append_event(db,tenant_id=tenant_id,event_type=f"{event_prefix}.requested",payload={"solution_id":row.id,"job_id":job.id,"version":version_id,"kind":source_kind},actor_type="owner",actor_id=user_id,source="solution_production")
        ...
        try:
            build_job,build_created=await self._job(db,tenant_id,solution_id,version_id,JobType.BUILD,key+":build")
            if build_created:transition(build_job,JobStatus.RUNNING,log="Rendering validated static source")
            if source:
                html=production_html(source,solution_id)
            else:
                schema=SiteSchema.model_validate_json(legacy.schema_json);html=render_site(schema,schema.pages[0].slug,"production").replace('/api/public/sites/production/forms/',f'/api/public/presence/{solution_id}/forms/')
            if build_created:transition(build_job,JobStatus.SUCCEEDED,evidence={"output_bytes":len(html.encode()),"source_version":version_id,"source_kind":source_kind},log="Static artifact rendered")
            verify_job,verify_created=await self._job(db,tenant_id,solution_id,version_id,JobType.VERIFY,key+":verify")
            ...
            if verify_created:transition(verify_job,JobStatus.SUCCEEDED,evidence={"source_kind":source_kind,"static_policy_valid":True},log="Artifact verification passed")
            result=await self.provider.deploy(solution_id=solution_id,version_reference=version_id,content=html);healthy,health=await self.provider.health(result)
            if not healthy:raise DeploymentFailure("Published artifact failed health verification")
            deployment=SolutionDeployment(tenant_id=tenant_id,solution_id=solution_id,job_id=job.id,provider=self.provider.name,provider_reference=result.provider_reference,version_reference=version_id,previous_deployment_id=current.id if current else None,public_slug=f"{solution_id[:12]}-{job.id[:8]}",public_url=f"/presence/{solution_id}",artifact_reference=result.artifact_reference,artifact_digest=result.artifact_digest,status="active",health_state="healthy",health_evidence_json=json.dumps(health,sort_keys=True),deployed_at=datetime.utcnow());db.add(deployment);await db.flush()
            if current:current.status="superseded"
            if source:
                await mark_published(db,tenant_id,runtime.id,source)
            else:
                old=await db.get(StudioVersion,runtime.published_version_id) if runtime.published_version_id else None
                if old and old.id!=legacy.id:old.status="superseded"
                legacy.status="published";legacy.published_at=datetime.utcnow();runtime.published_version_id=legacy.id
            runtime.status="published";row.lifecycle_status=LifecycleStatus.LIVE;row.production_state="live";row.production_url=deployment.public_url;row.current_version_reference=version_id;row.visibility="public";transition(job,JobStatus.SUCCEEDED,evidence={"deployment_id":deployment.id,"public_url":deployment.public_url,"health":health,"source_kind":source_kind},log="Health verification passed")
            await append_event(db,tenant_id=tenant_id,event_type=f"{event_prefix}.succeeded",payload={"solution_id":row.id,"job_id":job.id,"deployment_id":deployment.id,"version":version_id,"kind":source_kind,"public_url":deployment.public_url},source="solution_production");return job,row
        except (DeploymentUnavailable,DeploymentFailure,ValueError,CodingHarnessError) as error:
