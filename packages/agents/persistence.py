...
def _scope(metadata: dict[str, Any]) -> tuple[str, str, str | None, str | None]:
    ...
    if not user_id:
        # Truly anonymous runs remain trace-only. Durable state must always have a
        # principal/scope so it can never become a cross-user global bucket.
    ...
    async with session_scope() as db:
        ...
        if row is None:
            row = AgentRunRecord(
                ...
                conversation_id=(str(metadata.get("_conversation_id") or metadata.get("conversation_id") or "")[:255] or None),
            ...
            # A runtime_run_id is immutable to the scope that first created it.
            # Never move a checkpoint between Personal/Workspace owners.
