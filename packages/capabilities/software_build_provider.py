...
class SoftwareBuildProvider(BaseProvider):
    ...
    capabilities=(
        CapabilityDefinition("software.build","software_build","Create a canonical SoftwareProject and start a private durable build. Planning, coding, isolated build/test/start/health/acceptance and bounded repair are owned internally. Every project is immediately present in the Solutions Library.",{"type":"object","properties":{"objective":{"type":"string","minLength":1,"maxLength":12000},"name":{"type":"string","minLength":1,"maxLength":200},"return_source_archive":{"type":"boolean","default":True}},"required":["objective"],"additionalProperties":False},{"type":"object"},risk_level="medium",permissions=("solution:generate",),approval_policy=ApprovalPolicy.AUTO,reversible=True,plugin_id="operly.software",category="software",display_name="Build software",tags=frozenset({"software","application","codebase","source","studio","build","agent"}),semantic_operations=frozenset({"build an application","build complete software","create a working codebase","generate runnable source code","build and test software","create app source files","build software project for studio"})),
    ...
    async def execute(self,context,capability_name,arguments):
        if capability_name=="software.build":
            ...
            build_request={"requestedBy":context.actor_id,"returnSourceArchive":return_archive,"privatePreviewOnly":True,"publishAuthorized":False,"parentAgentRunId":parent_run,"origin":origin,"deliveryTarget":target}
            ...
            evidence=_job_evidence(job);evidence.update({"parentAgentRunId":parent_run,"returnSourceArchive":return_archive,"deliveryTarget":target});job.evidence_json=json.dumps(evidence,ensure_ascii=False,sort_keys=True,default=str)
            payload={"project":_project_json(project),"project_id":project.id,"solution":solution_json(solution),"solution_id":solution.id,"classification":{"runtime_type":"software_project","canonical":True},"job":_job_json(job),"job_id":job.id,"job_accepted":True,"build_state":job.status,"build_success":False,"source_archive_requested":return_archive,"private_preview_only":True,"publication_performed":False,"deployment_performed":False,"deferred":True,"continuation_kind":"software_build","parent_agent_run_id":parent_run}
