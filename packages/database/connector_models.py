...
class TenantConnector(Base):
    ...
    credential_reference:Mapped[str|None]=mapped_column(ForeignKey("connector_secrets.id"),nullable=True)
    ...
    created_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow)
    updated_at:Mapped[datetime]=mapped_column(DateTime,default=datetime.utcnow,onupdate=datetime.utcnow)
    __table_args__=(UniqueConstraint("tenant_id","provider","provider_account_id",name="uq_tenant_connector_account"),)
