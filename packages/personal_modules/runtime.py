...
from packages.personal_modules.google_completion_provider import completion_google_capabilities
...
from packages.personal_modules.reliable_google_provider import ReliablePersonalGoogleProvider
...
def build_personal_runtime(
    *,
    ...
    Production callers use the reliable Google provider, which extends the existing
    P3 completion provider with approval-bound Gmail send identity/reconciliation.
    Deterministic evaluation may inject a contract-compatible fake while still
    exercising the same registry, policy, idempotency, audit and Runtime1 path.
    """
    ...
    runtime.providers.register(
        ...
        personal_google_provider or ReliablePersonalGoogleProvider(),
    )
