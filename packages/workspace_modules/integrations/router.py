...
def _google_redirect_uri() -> str:
    return os.getenv("GOOGLE_OAUTH_REDIRECT_URI", "").strip() or (
...
async def _require_callback_owner(db: AsyncSession, data: dict) -> TenantMember:
    """Re-resolve fresh workspace authority after the provider redirect."""
    membership = await db.scalar(
        select(TenantMember).where(
            TenantMember.tenant_id == str(data["tenant_id"]),
    ...
    if membership is None or membership.role != "owner":
        raise HTTPException(
            403,
            "Workspace integration authority changed while authorization was in progress",
        )
    return membership


async def _upsert_google(
    db: AsyncSession,
    *,
    tenant_id: str,
    profile: dict,
    ...
    if row is None:
        row = TenantConnector(
            ...
            configuration_json=json.dumps(
                {"calendar_id": "primary", "display_name": account}
            ),
        ...
        config.update(
            {"calendar_id": config.get("calendar_id", "primary"), "display_name": account}
        )
...
async def _upsert_canva(
    ...
    *,
    ...
    config["display_name"] = str(
        identity.get("display_name") or "Canva account"
    )[:200]
    ...
    result.extend(
        {
            ...
            "health_status": (
                "healthy"
                if bot.is_ready() and bot.get_guild(int(row.external_space_id))
                else "offline"
            ),
...
async def google_connect(
    ...
    state = _serializer("google").dumps(
        {
            "ownership": "workspace",
...
async def canva_connect(
    ...
    url = canva_authorization_url(
        state=state, code_challenge=challenge, scopes=CANVA_SCOPES
    )
    await db.commit()
...
async def google_callback(code: str = Query(...), state: str = Query(...)):
    ...
    tokens["expires_at"] = datetime.now(timezone.utc).timestamp() + int(
    ...
    return RedirectResponse(
        f"/channels/{data['tenant_id']}/connections?connector=google",
        status_code=303,
    )
...
async def canva_callback(code: str = Query(...), state: str = Query(...)):
    ...
    async with session_scope() as db:
        ...
        await db.commit()
    return RedirectResponse(
        f"/channels/{data['tenant_id']}/connections?connector=canva",
        status_code=303,
    )
...
async def disable_connection(
    ...
    installation = await db.scalar(
        select(ChannelInstallation).where(
            ChannelInstallation.id == connector_id,
            ChannelInstallation.tenant_id == auth.tenant.id,
            ChannelInstallation.provider == "discord",
        )
    )
...
async def test_connection(
    ...
    if installation is None:
        raise HTTPException(404, "Connection not found")
    guild = (
        bot.get_guild(int(installation.external_space_id)) if bot.is_ready() else None
    )
    return {
        "ok": guild is not None,
        "health_status": "healthy" if guild is not None else "offline",
        "error": (
            None
            if guild is not None
            else "Discord bot is not connected to this server"
        ),
    }
