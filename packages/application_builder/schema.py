...
class FieldDefinition(Strict):
    ...
    def supported(self):
        ...
        if self.type == "status" and not self.options:
            raise ValueError("Status fields require options")
        return self
...
class ApplicationManifest(Strict):
    ...
    def references(self):
        ...
        for component in self.components:
            ...
            if component.type in {"Form", "DataTable"}:
                entity_id = component.properties.get("entityId")
                if component.type=="Form" and component.properties.get("authFlow") is True:
                    pass
                elif entity_id not in entities:
                    raise ValueError(f"{component.type} requires a known entityId")
            if component.type in {"TextInput","EmailInput","PasswordInput","Select","Checkbox","DateInput"}:
                parent = next((x for x in self.components if x.id == component.parentId), None)
                if parent and parent.properties.get("authFlow") is True:
                    continue
                entity = entities.get(parent.properties.get("entityId")) if parent else None
                if not entity or component.properties.get("fieldId") not in {field.id for field in entity.fields}:
...
class RecordInput(Strict):
    data: dict[str, Any]
    formId: str = Field(min_length=1, max_length=120)
    versionId: str = Field(min_length=1, max_length=36)
