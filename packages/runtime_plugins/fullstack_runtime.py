...
class FullStackRuntime:
    ...
    The source manifest never supplies shell commands, ports, registry hosts,
    secrets or resource limits. Those remain deterministic control-plane policy.
    """
    ...
    def detect(self, source: Any) -> RuntimeMatch:
        ...
        return RuntimeMatch(
            matched,
            1000 if matched else 0,
            ("operly full-stack manifest",) if matched else (),
        )
    ...
    def build_submission_from_record(
        ...
        dependencies = [
            Dependency(
                ...
                registry="npm" if item.ecosystem == "npm" else "pypi",
            )
        ...
        return BuildSubmission(
            ...
            healthCheck=HealthCheck(
                path=manifest.execution.healthPath,
                expectedStatus=200,
                timeoutSeconds=45,
            ),
            resources=self.spec.resource_policy,
            installNetwork=NetworkPolicy(
                mode="dependency_registry_only" if dependencies else "none"
            ),
    ...
    def build_submission(self, project, source, bindings):
        raise NotImplementedError(
            "Use the canonical persisted source build service; project-native submissions are a later migration"
        )
